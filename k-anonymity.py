...
def anonymize_data(id_data, qi_data, sensitive_data, k):
    # Step 1: Generalize Quasi-Identifiers
    qi_generalized = generalize_data(qi_data)
    
    # Step 2: Enforce k-anonymity on the quasi-identifiers
    qi_anonymized = enforce_k_anonymity(qi_generalized, k)
    
    # Step 3: Suppress sensitive attributes where necessary
    sensitive_suppressed = suppress_data(sensitive_data)
    
    # Step 4: Mask Identifiers
    ...
    anonymized_data = pd.concat([id_masked, qi_anonymized, sensitive_masked], axis=1)
...
if __name__ == "__main__":
    ...
    # Set the desired level of k-anonymity
    k = 3
    anonymized_data = anonymize_data(id_data, qi_data, sensitive_data, k)
